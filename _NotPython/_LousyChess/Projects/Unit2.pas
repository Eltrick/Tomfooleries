...
procedure TForm2.SizeBtnClick(Sender: TObject);
begin
  Big := Not(Big);
  if Big then
    ResetBig()
...
var
  Len : Integer;
begin
  while Length(Start) < 2 do
    Start := Start + IntToStr(Random(10));
  Len := Length(Start) - 1;
...
procedure TForm2.DrawMove(moveS : String; leave : Integer);
var
...
function TForm2.MakeMove(moveS : String; leave : Integer; NewElement:Boolean) : String;  //leave is important when backtracking
var
  Piece, x1, x2, y1, y2, tempPiece: Integer; //Piece that is being moved or replaced
  Promoted: Boolean;
  newS : String;
begin
  if NewElement then
  begin
    ...
    if abs(Piece) = 6 then
      KingAlive := False;
    tempPiece := StrToInt(Copy(moveS, 1, 2));
    x1 := StrToInt(moveS[3]);
    y1 := StrToInt(moveS[4]);
    x2 := StrToInt(moveS[5]);
    y2 := StrToInt(moveS[6]);
    if ((tempPiece = 1) and (y2 = 0)) or ((tempPiece = -1) and (y2 = hgt-1)) then
        tempPiece := tempPiece * 5;
    if Piece >= 0 then
      begin
        if tempPiece >= 0 then
          moveList[Length(moveList)-1] := '+' + IntToStr(tempPiece) + Copy(moveS, 3, 4) + '+' + IntToStr(abs(Piece))
        else
          moveList[Length(moveList)-1] := IntToStr(tempPiece) + Copy(moveS, 3, 4) + '+' + IntToStr(abs(Piece));
      end
    else
      if tempPiece >= 0 then
          moveList[Length(moveList)-1] := '+' + IntToStr(tempPiece) + Copy(moveS, 3, 4) + IntToStr(Piece)
      else
          moveList[Length(moveList)-1] := IntToStr(tempPiece) + Copy(moveS, 3, 4) + IntToStr(Piece);
  end;
  x1 := StrToInt(moveS[3]);
...
function TForm2.KingKilling(Reverse: Boolean; Number :Integer; White: Boolean):String;
var
...
function TForm2.KingDead(Reverse: Boolean; Number :Integer; White: Boolean):String; //Order: previous RNG; Number: Current RNG; White: is it white's turn?
var
  actual: Array of String;
...
function TForm2.LightLava(Reverse: Boolean; Number :Integer; White: Boolean):String; //Order: previous RNG; Number: Current RNG; White: is it white's turn?
var
...
function TForm2.DarkLava(Reverse: Boolean; Number :Integer; White: Boolean):String; //Order: previous RNG; Number: Current RNG; White: is it white's turn?
var
  First, Second: Array of String;
  i, Len: Integer;
  moveS : String;
...
function TForm2.Mirror(Reverse: Boolean; Number :Integer; White: Boolean):String; //Order: previous RNG; Number: Current RNG; White: is it white's turn?
var
  actual: Array of String;
...
function TForm2.Switch(Reverse: Boolean; Number :Integer; White: Boolean):String; //Order: previous RNG; Number: Current RNG; White: is it white's turn?
var
  actual: Array of String;
...
function TForm2.AnyMove(Reverse: Boolean; Number :Integer; White: Boolean):String;
var
  moveS: String;
...
procedure TForm2.AppendPart(Text:String);
var
  last : String;
  len : Integer;
begin
...
procedure TForm2.CalcBtnClick(Sender: TObject);
var
...
procedure TForm2.PrevBtnClick(Sender: TObject);
var
